...
class TransactionHandler:
    ...
    def perform_put(self, key, value):
        ...
        if key in self._lock_table:
            ...
            if self._lock_table[key][1] == [self._xid] or not self._lock_table[key][1]:
                ...
                self._lock_table[key][0] = 'X'
                if self._xid not in self._lock_table[key][1]:
                    self._lock_table[key][1].append(self._xid)
                self._store.put(key, value)
            
            elif (self._lock_table[key][0] == 'S' and self._lock_table[key][1]) \
                    or (self._lock_table[key][0] == 'X' and self._xid not in self._lock_table[key][1]) \
                    or self._lock_table[key][2]:
                self._desired_lock = ['X', key, value]
                
                if [self._xid, 'X'] not in self._lock_table[key][2]:
                    self._lock_table[key][2].append([self._xid, 'X'])
                return None
    ...
    def perform_get(self, key):
        ...
        # Part 1.1: your code here!
        if key in self._lock_table:
            if not self._lock_table[key][1] and not self._lock_table[key][2]:
                if [key, 'S'] not in self._acquired_locks:
                    self._acquired_locks.append([key, 'S'])
                self._lock_table[key][0] = 'S'
                self._lock_table[key][1].append(self._xid)
            elif (self._lock_table[key][0] == 'X' and self._xid not in self._lock_table[key][1]) \
                    or self._lock_table[key][2]:
                self._desired_lock = ['S', key]
                if [self._xid, 'S'] not in self._lock_table[key][2]:
                    self._lock_table[key][2].append([self._xid, 'S'])
                return None
            else:
                if [key, 'X'] and [key, 'S'] not in self._acquired_locks:
                    self._acquired_locks.append([key, 'S'])
                self._lock_table[key][0] = 'S'
                self._lock_table[key][1].append(self._xid)
        else:
    ...
    def release_and_grant_locks(self):
        ...
        for l in self._acquired_locks:
            ...
            key = l[0]
            lock = l[1]
            tid_list = self._lock_table[key][1] # all current transactions under the key
            if tid_list:
                for tid in tid_list:
                    if tid == self._xid:
                        tid_list.remove(self._xid)
            
            queue = self._lock_table[key][2] # all transactions in queue under the key
            if queue:
                for q in queue:
                    if q[1] == 'X':
                        if not tid_list:
                            self._lock_table[key][0] = 'X'
                            self._lock_table[key][1] = [q[0]]
                            queue.remove(q)
                        elif (self._lock_table[key][1] == [q[0]]):
                            self._lock_table[key][0] = 'X'
                            queue.remove(q)
                        break
                    if q[1] == 'S':
                        self._lock_table[key][0] = 'S'
                        self._lock_table[key][1].append(q[0])
                        queue.remove(q)
        self._acquired_locks = []
    ...
    def check_lock(self):
        ...
        if self._desired_lock == None:
            return None
        if self._desired_lock[0] == 'X':
            y = self.perform_put(self._desired_lock[1], self._desired_lock[2])
            if y:
                self._desired_lock = None
            return y
        elif self._desired_lock[0] == 'S':
            y = self.perform_get(self._desired_lock[1])
            if y:
                self._desired_lock = None
            return y
...
class TransactionCoordinator:
    ...
    def detect_deadlocks(self):
        ...
        # adds transactions to waits-for graph from the lock table
        waits_for = {}
        
        for key in self._lock_table.keys():
            if self._lock_table[key][1]:
                if self._lock_table[key][2]:
                    for cid in self._lock_table[key][1]:
                        for wid in self._lock_table[key][2]:
                            if wid[0] != cid:
                                if cid in waits_for:
                                    waits_for[cid].add(wid[0])
                                else:
                                    waits_for[cid] = set([wid[0]])
        
        # helper function
        path = set()
        
        def detect_cycles(xid):
            """
            [http://codereview.stackexchange.com/questions/86021/check-if-a-directed-graph-contains-a-cycle]
            
            input: graph d
            returns: True if the input graph d has a cycle
            """
            if xid in waits_for:
                path.add(xid)
                for waiting in waits_for[xid]:
                    if waiting in path or detect_cycles(waiting):
                        return True
                path.remove(xid)
            return False

        if waits_for:
            for v in waits_for.keys():
                isCycle = detect_cycles(v)
                #print(v, waits_for, isCycle)
                if isCycle:
                    return v
        return None
